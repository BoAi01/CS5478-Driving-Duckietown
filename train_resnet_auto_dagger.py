...
def _train(args):
    ...
    for epoch in range(args.epochs):
        ...
        actions, observations = list(actions.split(args.batch_size, dim=0)), list(
            observations.split(args.batch_size, dim=0))
        ...
        if epoch % 20 == 0:
            torch.save(model.module.state_dict(), f'{args.ckpt_name}-{args.map}.pt')

        if running_loss.avg > 0.02:
            continue
        ...
        observations_dagger, actions_dagger = [], []
        ...
        for i in range(1500):
            ...
            expert_action = torch.tensor(list(expert.predict(None)))
            observations_dagger.append(obs)
            actions_dagger.append(expert_action)
            obs, reward, done, info = env.step(action)
            total_reward += reward
            if total_reward < -3000:
                break

        observations_orig = torch.cat([observations_orig, observations_dagger], dim=0)
        actions_orig = torch.cat([actions_orig, actions_orig], dim=0)

        print(f'epoch {epoch}, train error {running_loss.avg:.5f}, roll out reward {total_reward:.3f}, '
              f'dataset length {len(observations_orig)}')
